...
program nia_stl
   ...
   character (128) :: output_fileSTL="STL_INPUT_fnia1.stl"
   character (128) :: output_fileSTL_csytle="STL_INPUT_fnia0.stl"
   character (128) :: output_fileNNB="NNBindex.dat"
   character (128) :: vertex_fileNNB="NNBvertx.csv"
   character (128) :: facets_fileNNB="NNBfacet.dat"
   character (128) :: title_stl
   character (128) :: prefix_nnbed="fnia_", postfix_nnbed="_fnia"
   character (128) :: postfix_cstyle="_cstyle" , postfix_fstyle="_fstyle" 
   ...
   if ( nargc == 0 )  then
      ...
      output_fileSTL       =&
           trim(adjustl(arg1_root))//trim(adjustl(postfix_nnbed))//trim(adjustl(postfix_fstyle))//trim(adjustL(arg1_extn))
      output_fileSTL_csytle=&
           trim(adjustl(arg1_root))//trim(adjustl(postfix_nnbed))//trim(adjustl(postfix_cstyle))//trim(adjustL(arg1_extn))
   ...
   write(*,*) "Input  STL file: ", trim(adjustL(entree_fileSTL))
      write(*,*) "Output STL file - F style: ", trim(adjustL(output_fileSTL))
      write(*,*) "Output STL file - C style: ", trim(adjustL(output_fileSTL_csytle))
   ...
   allocate(myFacet		(numtri))
   allocate(loadedFacet		(numtri))
   allocate(myNNbor		(numtri))
   allocate(loadedNNbor		(numtri))
   allocate(nnbedFacet		(numtri))
   allocate(numNborFacets       (numtri))
   allocate(numNborVertices     (numtri))
   ...
   call write_ascii_stl_w_nnb_all_cstyle(output_fileSTL_csytle  ,myFacet,myNNbor,numtri,title_stl)
