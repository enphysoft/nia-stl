...
!*******************************************************************
   ...
   subroutine write_ascii_stl_w_nnb_all (outputfile,hx,hxnnb,numtri,title)
    ...
    subroutine write_ascii_stl_w_nnb_each (ifct,outputfile,lin,hx1,hxnnb1)
      ...
      write(mynumstring,*) ifct         ! 1. the facet's id, i.e., sequential index. 
      WRITE(lin,"(A)",advance="no") trim(adjustl(mynumstring))  ! 1. the facet's the three primary nearest neighbors. 
      ...
      do j = 1, hxnnb1%vidPairMax
         write(mynumstring,*) hxnnb1%vidPair(j)  ;       ! 1. the facet's the multiple secondary nearest neighbors. 
   ...
   subroutine write_ascii_stl_w_nnb_all_cstyle (outputfile,hx,hxnnb,numtri,title)
      implicit none 
      ! integer,                  intent(in)      :: Nx,Ny
      character(len=4)     :: Cloop
      character(len=4)     :: Cfacet  
      ...
      OPEN(lin, file=outputfile, status='replace')
         ...
         write(*,*) title
         ifct_loop: do ifct = 1, numtri 
            call  write_ascii_stl_w_nnb_each_cstyle (ifct,outputfile,lin,hx(ifct),hxnnb(ifct))
          enddo ifct_loop
         WRITE(lin,"('endsolid',2X,A128)") title 
      CLOSE(lin)
    end subroutine write_ascii_stl_w_nnb_all_cstyle
   
    !****************************************************************

    subroutine write_ascii_stl_w_nnb_each_cstyle (ifct,outputfile,lin,hx1,hxnnb1)
      implicit none 
      type(stl_tri), intent(inout) :: hx1 
      type(stl_nnb), intent(inout) :: hxnnb1
      integer      , intent(in)    :: ifct
      ...
      WRITE(lin,"(4x,'endloop')")
      WRITE(lin,"(4x,'endfacet',4x)",advance="no") 
      write(mynumstring,*) ifct - 1        ! 1. the facet's id, i.e., sequential index. 
      WRITE(lin,"(A)",advance="no") trim(adjustl(mynumstring))  ! 1. the facet's the three primary nearest neighbors. 
      write(mynumstring,*) hxnnb1%fidPair(1) - 1 ; WRITE(lin,"(',',A)",advance="no") trim(adjustl(mynumstring))
      write(mynumstring,*) hxnnb1%fidPair(2) - 1 ; WRITE(lin,"(',',A)",advance="no") trim(adjustl(mynumstring))
      write(mynumstring,*) hxnnb1%fidPair(3) - 1 ; WRITE(lin,"(',',A)",advance="no") trim(adjustl(mynumstring))
      do j = 1, hxnnb1%vidPairMax
         write(mynumstring,*) hxnnb1%vidPair(j) - 1 ;       ! 1. the facet's the multiple secondary nearest neighbors. 
         WRITE(lin,"(',',A)",advance="no") trim(adjustl(mynumstring))
      enddo
      WRITE(lin,*)
    end subroutine write_ascii_stl_w_nnb_each_cstyle
